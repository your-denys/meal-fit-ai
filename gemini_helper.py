...
def get_reminder_suggestion(
    ...
    """
    Короткое напоминание «пора поесть» с конкретным советом, что съесть для недобора.
    hour — текущий час (0-23); учитывается: после 21:00 не тяжёлое, утром — разумные порции.
    """
    ...
    eaten_block = f" Уже ели сегодня: {', '.join(eaten_today[:15])}." if eaten_today else ""

    prompt = f"""Ты нутрициолог. Напоминание пользователю «пора поесть» с учётом недобора за день.
...
Недобор: калории {cal_rem:.0f}, белок {prot_rem:.0f} г, углеводы {carb_rem:.0f} г.{eaten_block}

Время: сейчас {hour}:00. {time_context}
Не предлагай блюда, которые пользователь уже ел сегодня — предложи другое.
