...
async def handle_clarification(message: Message, state: FSMContext):
    ...
    full_prompt = f"Пользователь хотел добавить: '{original_text}'. Gemini распознал это как '{original_name}'. Пользователь уточняет: '{message.text}'. Рассчитай итоговое КБЖУ с учётом всего контекста."
    result = analyze_food_text(full_prompt)
    if not result or result.get("needs_clarification"):
        await message.answer("Понял, считаю...")
